...
- **Blocking Dialogs**:
  ...
  - **Share Guard**: If a modal includes a "Share via WhatsApp" or "Copy" action for notifications:
    - The **X button MUST always be visible** (`:closable="true"`). NEVER use `:closable="false"` or `:closable="canClose()"` to hide it â€” this removes the button entirely.
    ...
    - **ALWAYS use the `useShareReminder` composable** (`@/composables/useShareReminder.ts`) â€” never duplicate share/copy/guard logic locally.
...
- **Code Redundancy**:
  - WhatsApp share logic (share, copy, guard, reset) MUST live in `useShareReminder` composable â€” never re-implement locally.
...
## ðŸš€ Skill Activation (Auto-Load)
...
1. **Surface it immediately** â€” say:

   > ðŸ’¡ **KI Candidate detected**: This looks like knowledge worth persisting (e.g., `[pattern name]`). Want me to save this to persistent memory?

2. **If user confirms** â€” create the KI automatically:
...
3. **If user declines** â€” respect it and move on.
...
knowledge/
â””â”€â”€ {kebab-slug}/
    â”œâ”€â”€ metadata.json      â† title, summary, tags, lastUpdated, references
    â””â”€â”€ artifacts/
        â””â”€â”€ convention.md  â† full detail, code examples, edge cases
```

## âš ï¸ Do NOT Flag These
...
- Works alongside `continuous-improvement.md` â€” if a manual task is repeated 3+ times, that's also a KI candidate.
- Complements `session-summary-protocol.md` â€” at session end, do a final sweep for any unflagged KI candidates.
...
// âŒ AVOID: Always opening new tabs unnecessarily
```
...
1. **Natural stopping points:**
   - âœ… User says "done for today", "let's stop", "continue tomorrow"
   - âœ… Late hours (after 11 PM local time)
   - âœ… Session duration > 90 minutes
   - âœ… Major milestone completed (phase, feature, deployment)

2. **Significant progress:**
   - âœ… 3+ major tasks completed
   - âœ… New project initialized
   - âœ… Complex feature shipped
   - âœ… Multiple workflow improvements made

3. **User explicitly requests:**
   - âœ… "Create a summary"
   - âœ… "What did we accomplish?"
   - âœ… "Recap today's work"
...
- ðŸ“Š Progress bars for quantifiable progress
- ðŸŽ¨ Emojis for visual categorization
...
- âœ… **Emojis:** Use liberally for scannability
- ðŸ“Š **Tables:** For comparisons, decisions, metrics
- âœ… **Progress bars:** ASCII bars for percentages
- ðŸŽ¯ **Headers:** Clear hierarchy (H1 â†’ H6)
- ðŸ“ **Code blocks:** For commands, file structures
...
1. **Major Project Milestones**
   - âœ… Full system integration complete (like Skills Library)
   - âœ… Production deployment successful
   - âœ… Complex feature shipped to users
   - âœ… Major refactoring/architecture upgrade complete

2. **Learning Breakthroughs**
   - âœ… New technology stack mastered
   - âœ… Complex problem solved after significant effort
   - âœ… Performance optimization with measurable impact

3. **Workflow Achievements**
   - âœ… Automation system built that saves 10+ hours/week
   - âœ… Complete CI/CD pipeline established
   - âœ… Development environment fully optimized
...
ðŸ† Achievement Status: [SUCCESS LEVEL]
```
...
- Phase completion (e.g., Phase 1 â†’ 100%)
...
- [ ] âœ… All major accomplishments listed
- [ ] ðŸ“Š Metrics/numbers included (time, files, lines, %)
- [ ] ðŸŽ¯ Clear next steps defined
- [ ] ðŸŽ¨ Visual elements (emojis, tables, progress bars)
- [ ] ðŸ“ Links to relevant files/docs
- [ ] ðŸ’¡ Lessons learned (if applicable)
- [ ] ðŸš€ Productivity insights (if workflow changes made)
- [ ] ðŸ§  KI Sweep: Any patterns/conventions from this session worth persisting? Suggest `/save-ki` if yes.
...
1. **Clarity** â€” Users should immediately understand the interface
2. **Consistency** â€” Similar elements should look and behave similarly
3. **Feedback** â€” Every action should have a visible response
4. **Efficiency** â€” Minimize steps to complete tasks
5. **Accessibility** â€” Design for all users
...
- âœ… PHP/Node/Python support
- âœ… Auto-deploy from Git
- âš ï¸ Cold starts after 15 min inactivity (use cron ping)
...
- âŒ Vercel (stick to one platform)
- âŒ Netlify (stick to one platform)
- âŒ GitHub Pages (limited SPA support)
...
**User Background:** Former graphic/web designer â†’ highly visual/graphical person
...
- âœ… Emoji in header or status
- ðŸ“Š Table if 2+ items
- ðŸ§­ Clear sections (H2/H3)
- ðŸ·ï¸ Code blocks labeled with file path
...
| Error / alert         | âŒ + Issue + Fix block     |
...
1. **Status indicators**
   ...
   - ðŸŽ¯ Goal / Target
   - ðŸš€ Launch / Deploy
   - ðŸŽ‰ Celebration / Major milestone
...
Write-Host "ðŸ”„ Processing..." -ForegroundColor Cyan
Write-Host "âŒ Error occurred" -ForegroundColor Red
...
| Item   | Status         | Notes             |
| ------ | -------------- | ----------------- |
| Setup  | âœ… Done        | All files created |
| Config | ðŸ”„ In progress | 80% complete      |
```
...
- ðŸŽ¨ **Design** â†’ Beautiful, premium aesthetics
- ðŸš€ **Deploy** â†’ Cloudflare Pages
- ðŸ“Š **Data** â†’ TiDB Cloud
...
| Item      | Status  | Notes        |
| --------- | ------- | ------------ |
| Feature X | âœ… Done | Brief note   |
| Feature Y | ðŸ”„ WIP  | ETA tomorrow |
...
1. ðŸŽŠ Party Header with celebration
2. âœ… Verified Status list
3. ðŸ“Š Visual Progress Table
4. ðŸŽ¯ Strategic Summary
5. ðŸš€ Next Strategic Steps
6. ðŸ† Certification Footer
...
1. âœ… Use visual hierarchy (headers, tables, lists)
2. ðŸŽ¨ Include relevant emojis for scannability
3. ðŸ“Š Present data in structured format
4. ðŸŽŠ Use the **Rich Achievement Style** for all final project milestones
5. ðŸš€ Feel engaging, not boring
6. ðŸŽ¯ Match format to response length (don't overdo short answers)
...
## ðŸ”§ Built-In Dev Simulator Tools
...
- Access via Dev Tools panel â†’ "WA Message Preview" button
...
- **7 Scenarios:** Pending unassigned â†’ Tester assigned â†’ Tester passed â†’ Rejected by Tester â†’ Rejected by SA/BA â†’ Verified normal â†’ Verified technical
...
- **Reactive:** Computed live from `useDeploymentPermissions` â€” any code change to the composable reflects instantly
- Access via Dev Tools panel â†’ "Role Visibility Sim" button

---

## ðŸ›¡ï¸ Security: Localhost-Only Access

Both simulator pages are gated behind a **double lock**:

1. **Router meta:** `meta: { devOnly: true }` â€” router guard checks `import.meta.env.DEV`
...
| ----------------------------------------- | ------------------- | ------------------------------------------------- |
| **Changed `useDeploymentPermissions.ts`** | Role Visibility Sim | Verify all 7 scenarios Ã— 7 personas still correct |
...
2. Open Dev Tools panel â†’ click "Role Visibility Sim"
3. Click through ALL 7 scenarios
4. Verify each role card shows correct âœ…/â€” for every permission
...
- âŒ Claim permission logic works without checking the simulator
- âŒ Manually test by switching personas one-by-one (use the matrix view)
- âŒ Skip scenarios â€” edge cases hide in "Tester (Not Assigned)" and "Developer (Other)"
- âŒ Forget to check both `shareButton` AND `assignmentShare` columns
...
- Shows every role as a card with permission rows (âœ… Yes / â€” No)
- Computes permissions live from the actual composable (single source of truth)
- Uses `computePermsFor(entity, persona)` pattern: temporarily override auth store â†’ read permissions â†’ restore
...
â”œâ”€â”€ views/pages/    # Route-level components
â”œâ”€â”€ hooks/          # Reusable logic (React)
â”œâ”€â”€ composables/    # Reusable logic (Vue)
...
- âœ… Use search: "How does Vue 3.5's new `useId()` composable work?"
- âœ… Use search: "Did Laravel 11 deprecate the `$dates` property?"
- âŒ No search needed: "How do I use Vue's `computed()` in Composition API?" (stable feature)
...
5. STAFF CHECK: Ask yourself â€” "Would a staff engineer approve this?"
...
```
âœ… "Could a senior engineer read this without frowning?"
âœ… "Is there a more elegant solution I'm skipping due to laziness?"
âœ… "Does this fix the root cause or just mask the symptom?"
âœ… "Would I be comfortable if this went live as-is?"
âŒ Shipping a hacky fix because "it works for now"
âŒ Skipping elegance check on complex logic changes
```
...
- Vite (Vue): `npm run dev` â†’ runs on port 3000
...
1. **DRY (Don't Repeat Yourself)**
   - If you write the same code/script twice â†’ extract it to a reusable module
   - If you give the same instruction twice â†’ create a workflow

2. **Automation First**
   - Manual task done 3+ times â†’ automate it
   - User has to copy/paste long code â†’ create a script file
   - Multi-step process â†’ create a workflow

3. **Centralize & Modularize**
   - Scripts â†’ `$env:USERPROFILE\.gemini\.agent-templates\scripts\`
   - Rules â†’ `$env:USERPROFILE\.gemini\.agent-templates\workspace\rules\`
   - Workflows â†’ `$env:USERPROFILE\.gemini\global_workflows\`
...
4. **Implement immediately**: Don't just suggestâ€”do it (with permission)
...
- âœ… Proactively identify optimization opportunities
- âœ… Suggest better approaches when you see them
- âœ… Update workflows/scripts as you learn better patterns
- âœ… Balance improvement suggestions with current task focus

## Skill Activation Standard (PROACTIVE)

**Rule:** When **creating or modifying** any rule that references skills, **ALWAYS** include a `ðŸš€ Skill Activation (Auto-Load)` table.

### Auto-Check Triggers:

- âœ… **Creating a new rule** â†’ Check if it needs skill wiring
- âœ… **Modifying an existing rule** â†’ Check if a Skill Activation table is missing
- âœ… **Rule references a skill inline** â†’ Extract to a proper table
- âœ… **Rule has a "Related Skills" bullet list** â†’ Convert to standardized table format
...
- âœ… After adding a new specialized skill (e.g., "Is this wired to the right Expert Rule?")
- âœ… When a rule feels "weak" or "generic" (e.g., "Should I link this to a Pro skill?")
- âœ… Periodically (~every 10 sessions) check for broken or outdated links
...
- ðŸ”´ **Critical** â€” App crashes, data loss, security issue
- ðŸŸ¡ **Warning** â€” Functionality broken, poor UX
- ðŸŸ¢ **Suggestion** â€” Improvement opportunity, minor issue
...
| **Production Code Audit**  | `.agents/skills/production-code-audit/SKILL.md`   | Pre-deploy hardening â€” line-by-line quality gate       |
...
- ðŸš© Copy-pasting code blocks between files
- ðŸš© Same `computed()` or function in multiple components
- ðŸš© Identical formatting logic (dates, names, currencies)
- ðŸš© Same validation rules in multiple places
- ðŸš© Repeated API response transformations
...
- âŒ Create "utils" files with single functions (inline instead)
- âŒ Over-abstract for only 2 occurrences (wait for 3+)
- âŒ Extract without verifying build passes
- âŒ Ignore TypeScript type implications
- âŒ Leave old imports/code in place after extraction

## Success Metrics

After refactoring, you should see:

- âœ… Single import source for the logic
- âœ… Reduced total lines of code
- âœ… Easier future maintenance
- âœ… Consistent behavior across all usage points
- âœ… Build and lint still pass
...
âš ï¸ REDUNDANCY DETECTED - 4 identical functions
...
- **Critical (State Management, Complex Logic, Data Flow)**:
  ...
  - **Sub-Chunk Mode (For deep learning)**:
    ...
    - **Example order**: Imports â†’ Props/Emits â†’ Computed 1 â†’ Computed 2 â†’ Template Header â†’ Template Body â†’ Styles
...
- **Report Results**: Clearly state what was verified and the results (e.g., "âœ… Type-check passed, 0 errors")
...
- "Awesome progress on the Tailwind setup! ðŸŽ‰ Take 5?"
...
Always create a `ROADMAP.md` file in the project root when starting a new project. Use this format for visual progress tracking.

## AUTO-UPDATE RULE (CRITICAL)

**You MUST automatically update `ROADMAP.md` whenever:**

1. âœ… **Complete a significant task** - Mark task as `[x]` and recalculate progress bar
2. âœ… **Finish a phase** - Update status header, mark phase complete (100%)
3. âœ… **Major milestone achieved** - Update Key Milestones section
4. âœ… **Add new features/tasks** - Add to appropriate phase
5. âœ… **Change current phase** - Update status header to new phase
6. âœ… **Deploy to production** - Mark deployment phase complete

**Update simultaneously with:**
- Completing implementation work
- Before calling `notify_user` on major progress
- After finishing any task in `task.md`

**Never skip updating ROADMAP** - It's as important as the code itself for tracking progress.
...
> **Current Phase:** Phase 1 - {Current Phase Name}  
...
\```
Phase 1: {Phase Name}       â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘ 80%
Phase 2: {Phase Name}       â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘  0%
Phase 3: {Phase Name}       â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘  0%
Phase 4: {Phase Name}       â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘  0%
\```
...
- ðŸŽ¯ {Target milestone}: TBD
```

## Progress Bar Format

**Use exactly 10 characters for the progress bar:**
- `â–ˆ` = completed (filled block)
- `â–‘` = remaining (light shade)

**Examples:**
- `â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘` =   0% (0/10)
- `â–ˆâ–ˆâ–‘â–‘â–‘â–‘â–‘â–‘â–‘â–‘` =  20% (2/10)
- `â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘â–‘â–‘` =  50% (5/10)
- `â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘` =  80% (8/10)
- `â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ` = 100% (10/10)

## Phase Emojis

Use emojis to make phases more visual and scannable:
...
1. Complete a task (mark with `[x]`)
2. Update progress bars (recalculate percentage)
3. Move to a new phase (update status header)
4. Add new tasks or phases
5. Complete a milestone

## Why This Format?

- **Visual progress** - Bars give instant status overview
- **Scannable** - Emojis and formatting make it easy to skim
- **Motivating** - Seeing progress bars fill up is satisfying
- **Lightweight** - Quick to update without diving into specs
- **Team-friendly** - Stakeholders can see progress at a glance
...
1. Does this need implementation? â†’ Defer to specialist
2. Is this a strategic choice? â†’ Advise directly
3. Is there a conflict between roles? â†’ Help resolve it
4. Is the user stuck? â†’ Provide options and trade-offs
...
This file is **global** â€” lessons learned in one project automatically apply to ALL future projects. It compounds over time.
...
| Trigger                                         | Example                             |
| ----------------------------------------------- | ----------------------------------- |
| User explicitly corrects a mistake              | "That's wrong, you should haveâ€¦"    |
| User says "no", "that's not right", "actuallyâ€¦" | Redirect mid-task                   |
...
**Write the lesson IMMEDIATELY** â€” before continuing any other task.
...
- `Verification` â€” false completion claims, wrong assumptions
- `Code Redundancy` â€” duplicate logic, copy-paste violations
- `Git` â€” wrong branch, missing commits, bad messages
- `UI` â€” visual assumptions, browser testing issues
- `Files` â€” wrong file edited, path confusion
- `Context` â€” missed breadcrumbs, session state ignored
- `Architecture` â€” wrong pattern, premature optimization
- `Communication` â€” user had to repeat themselves
...
2. âœ… NEW: Read tasks/lessons.md â†’ scan for patterns relevant to TODAY's task
3. Check STATE.json / NEXT.md breadcrumbs
4. Load task context
```

**Scan heuristic:** Read all entries. If a category matches today's work (e.g., working on UI â†’ check UI lessons), flag those lessons as "active context" for the session.
...
| ------------------------ | --------------------------------------------------- |
| `failure-postmortem`     | After logging failure â†’ also append to `lessons.md` |
| `verification-protocol`  | After Iron Law violation â†’ new lesson entry         |
| `continuous-improvement` | Repeated pattern spotted â†’ new lesson entry         |
| `update-breadcrumbs`     | End of session â†’ commit any lessons added today     |
...
- Reproducible â€” another AI would understand and avoid it

**Bad lesson entry (too vague):**

- âŒ "Be more careful"
- âŒ "Think before coding"
- âŒ "Verify things"
...
---
## 🧠 Synced Brain Artifacts


The following documents provide the most recent context from the Antigravity AI brain.


### implementation_plan.md


([link](file:///D:/Coding Zone/Vue/sem-ssl-expiry-monitor/.agents/docs/implementation_plan.md))


### task.md


([link](file:///D:/Coding Zone/Vue/sem-ssl-expiry-monitor/.agents/docs/task.md))
